...
from pde import *
from modelPINN import create_pinn_model
...
brain_id = "111312"
input_dir = "../Brains"
output_dir = "../Checkpoints_PINN_256_512_512_256_128"  # Include PINN and architecture in name
gpu_device = -1
epoch_num = 200  # 10000
...
# PINN architecture: hidden layers
PINN_HIDDEN_LAYERS = [256, 512, 512, 256, 128]  # Equivalent to ~600K-800K parameters
PINN_ACTIVATION = 'tanh'
PINN_USE_POSITIONAL_ENCODING = True
PINN_ENCODING_DIM = 64
# --------------------------------


def create_coordinate_grid(shape, device='cpu'):
    """
    Create coordinate grid for PINN input.
...
def plot_loss():
    ...
    ax.set_title(
        ...
        f"epochs={loss_df['epoch'].iloc[-1]} | lr={learning_rate} | PINN layers={PINN_HIDDEN_LAYERS}",
        fontsize=14,
        pad=10
    )
...
def train(brain_id, input_dir, output_dir, gpu_device=0, epoch_num=10000, learning_rate=1e-4,
    ...
    config_data.update({
        ...
        "use_positional_encoding": PINN_USE_POSITIONAL_ENCODING,
        "encoding_dim": PINN_ENCODING_DIM,
        "resume": resume,
    })
    ...
    for epoch in tqdm(range(start_epoch, epoch_num)):
        ...
        for i, batched_id_sample in enumerate(dataloader_id):
            ...
            # PINN forward: takes coordinates and vector field
